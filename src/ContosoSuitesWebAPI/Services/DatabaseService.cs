...
public class DatabaseService(string connectionString) : IDatabaseService
{
    ...
    /// </summary>
    [KernelFunction]
    [Description("Get all bookings for a single hotel starting from a specific date and time.")]
    public async Task<IEnumerable<Booking>> GetBookingsByHotelAndMinimumDate([Description("The ID of the hotel")] int hotelId, [Description("The starting date and time")] DateTime dt)
    {
    ...
    [KernelFunction]
    [Description("Get all bookings that has more than one room.")]
